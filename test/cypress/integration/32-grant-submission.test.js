describe('Grant Submission Flow', () => {
  ...
  it('should allow users to submit a grant application', () => {
    ...
    // Add a new payout method - PayPal
    ...
    cy.get('[data-cy="payout-other-info"]').type('Please send the funds to my bank account: xxxxxxxxxxx');
    ...
    // Fill in the grant description
    cy.get('[data-cy="expense-description"]').type('Grant application for community project');

    // Fill in the amount field
    cy.get('[data-cy="amount-input"]').type('{selectall}1000');
    ...
    cy.contains('test-paypal@example.com').should('be.visible');
